...
class Database
{
   ...
   // private $dsn = "sqlsrv:Server=localhost;Database=test";    // Conect with SQLServer
    ...
    {
        $data = array();
        $sql = "SELECT id, username, email, phone, company FROM recruiter order by id DESC";
    ...
    { 
        $sql = "SELECT id, username, email, phone, company FROM recruiter WHERE id=:id";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute(['id' => $id]);
    ...
    {
        $sql2 = "DELETE FROM recruiter WHERE id=:id";
    ...
    {
        $sql = "SELECT count(*)  FROM recruiter";
